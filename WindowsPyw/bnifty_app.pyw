...
label_nifty = tk.Label(left_frame, text="BNIFTY", font=("Arial", 9))
...
btn_sl = tk.Button(left_frame, text="SL", command=lambda: run_asyncio_task(manual_sl(action="BNF_CE_SL",value=stopLoss.get())), bg="pink")
btn_sl.grid(row=8, column=3, padx=10, pady=10)

btn_reset = tk.Button(left_frame, text="RST", command=lambda: run_asyncio_task(manual_sl(action="BNF_CE_SL",value="0")), bg="blue")
...
label_nifty = tk.Label(right_frame, text="BNIFTY", font=("Arial", 9))
label_nifty.grid(row=8, column=1, padx=10,pady=10)

nftsl = tk.Entry(right_frame,width=10,textvariable=stopLoss)
nftsl.grid(row=8, column=2, padx=10,pady=10)

btn_sl = tk.Button(right_frame, text="SL", command=lambda: run_asyncio_task(manual_sl(action="BNF_PE_SL",value=stopLoss.get())), bg="pink")
btn_sl.grid(row=8, column=3, padx=10, pady=10)

btn_reset = tk.Button(right_frame, text="RST", command=lambda: run_asyncio_task(manual_sl(action="BNF_PE_SL",value="0")), bg="blue")
btn_reset.grid(row=8, column=4, padx=10, pady=10)
