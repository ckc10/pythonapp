...
def send_post_request(action, value="1"):
    data = {
    ...
    if value == "1":
        if action.startswith("Buy") or action.startswith("Sell") or action.startswith("CE"):
...
def reset_all():
    actions = ["Buy_CE", "Sell_CE", "CE_Sell_1", "CE_Sell_2", "CE_Sell_3", "CE_Exit", "UN_Exit",
               "Buy_PE", "Sell_PE", "PE_Sell_1", "PE_Sell_2", "PE_Sell_3", "PE_Exit", "UN_Exit"]
    for action in actions:
        send_post_request(action, value="0")
...
button_u_exit = tk.Button(left_frame, text="U.Exit", command=lambda: send_post_request("UN_Exit"), bg="grey")
...
# Adding the PE side code as well 
# First line: Buy, Sell
...
button_u_exit = tk.Button(right_frame, text="U.Exit", command=lambda: send_post_request("UN_Exit"), bg="grey")
button_u_exit.grid(row=3, column=8, padx=10, pady=10)
...
reset_button = tk.Button(root, text="Reset", command=reset_all, bg="yellow")
